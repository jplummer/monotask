...
**Recommended:** keep team in **[`Monotasker/Config/MonotaskerSigning.local.xcconfig`](Monotasker/Config/MonotaskerSigning.local.xcconfig)** (gitignored). Each **`xcodegen generate`** runs a **post-generate** step: if that file does not exist, it is **copied** from [`MonotaskerSigning.local.xcconfig.example`](Monotasker/Config/MonotaskerSigning.local.xcconfig.example).
...
3. The wrapper [`Monotasker/Config/MonotaskerSigning.xcconfig`](Monotasker/Config/MonotaskerSigning.xcconfig) `#include?`s that file; it is **not** overwritten by `xcodegen`.
...
1. Set `CFBundleDisplayName` in [`Monotasker/App/Info.plist`](Monotasker/App/Info.plist) (or override via `project.yml` `INFOPLIST_KEY_CFBundleDisplayName`).
